# Macedonian Daily Lesson Structure
...
## Mandatory tracker-integrity workflow
...
5. Re-read the updated tracker sections and verify both reconciliation markers match before teaching.

### After every completed lesson

1. Record newly encountered and practised vocabulary in `macedonian_vocabulary.md`.
2. Record grammar taught, demonstrated, corrected, or requiring review in `macedonian_grammar.md`.
3. Add brief dated lesson evidence to both trackers.
4. Set **Last completed lesson reconciled** and **Last reconciliation performed** in both files to that lesson's date.
5. Re-read the written sections and verify the changes exist and the markers agree.
6. Never state that progress was saved, reconciled, or persisted unless the write and verification actually succeeded.
...
- New material must become progressively more challenging as demonstrated ability grows. “New” means not already understood from an earlier pattern or familiar phrase.
- Before teaching a proposed concept, check the trackers and lesson evidence for prior knowledge. If the learner already knows it—for example, `од` from `Јас сум од Австралија`—use it only as a brief bridge or contrast and advance to a genuinely new concept.
- Select material at the learner's current edge: build on secure patterns while adding one meaningful new demand such as a new verb-person contrast, question pattern, case/preposition use, adjective agreement, possession, past/future construction, or longer clause.
- Each normal lesson must contain at least one **stretch prompt** that combines two or more older patterns with the new concept and provides less scaffolding.
